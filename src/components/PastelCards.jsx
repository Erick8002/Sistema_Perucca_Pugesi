...
export default function PastelCards({ transactions = [] }) {
  ...
  const highValueTransactions = useMemo(
    () =>
      transactions.filter(
        (item) => parseCurrency(item.valor) > Number(thresholdValue || 0),
      ),
  ...
  const employeeTransactions = useMemo(
    () =>
      transactions.filter((item) => {
        ...
        return categoria.includes("func") || categoria.includes("holeri");
      }),
    [transactions]
  );
  ...
  return (
    <div className="w-full overflow-hidden">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map(
          ({ title, value, subtitle, style, editableValue, details = [] }) => {
                ...
                <div key={title} className="space-y-3">
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 text-[11px] text-gray-500 sm:text-xs">
                      <span className="leading-snug font-medium text-gray-600">
                        {title}
                      </span>
                      <label
                      ...
                      >
                        <span className="text-[9px] font-bold tracking-[0.08em] text-purple-700">
                          R$
                        </span>
                        <input
                          type="number"
                          ...
                          onChange={(event) =>
                            setThresholdValue(Number(event.target.value) || 0)
                          }
                  ...
                  {isExpanded && (
                    <div
                      className={`rounded-xl border border-white/60 p-4 shadow-[0_8px_24px_rgba(15,23,42,0.06)] backdrop-blur-sm ${tones[style].panel}`}
                    >
            ...
            return (
              <div key={title} className="space-y-3">
                <div
                  ...
                  className={`relative min-h-[170px] cursor-pointer rounded-xl border p-4 shadow-sm transition-shadow duration-200 hover:shadow-md sm:p-5 ${tones[style].card}`}
                >
                  <span className="block text-[11px] font-medium text-gray-600 sm:text-xs">
                    {title}
                  </span>
